...
#include <Library/TpmCommLibNew.h>
...
HashLogExtendEvent (
  ...
  IN      TIS_TPM_DEVICE_PTR            TpmHandle,
  ...
  )
{
  ...
  TCG_PCR_EVENT_HDR                 TcgEventHdr;
  TIS_TPM_DEVICE                    TpmHandle;

 /* TpmHandle = (TIS_TPM_DEVICE) (UINTN) TPM_BASE_ADDRESS; */
  TisTpmHandle(&TpmHandle);
  ...
  )
{
  ...
  Status = TisPcRequestUseTpm (&TpmHandle);
  if (EFI_ERROR (Status)) {
    return Status;
  }
