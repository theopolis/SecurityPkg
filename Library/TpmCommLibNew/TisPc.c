...
extern TIS_TPM_DEVICE TIS_TPM_I2C_ATMEL;

STATIC TIS_TPM_DEVICE TIS_TPM_DEVICE_NULL = {
		/* Not a thing */
};
...
EFIAPI
TisTpmHandle (
  OUT	TIS_TPM_DEVICE_PTR 	TisReg
  )
{
  switch (PcdGet8 (PcdTpmDeviceType)) {
    case TIS_DEVICE_TYPE_I2C_ATMEL:
    	*TisReg = (TIS_TPM_DEVICE) TIS_TPM_I2C_ATMEL;
      break;
    default:
    	*TisReg = (TIS_TPM_DEVICE) TIS_TPM_DEVICE_NULL;
      return 1;
   }
  return 0;
}
...
BOOLEAN
TisPcPresenceCheck (
  IN      TIS_TPM_DEVICE_PTR      TisReg
  )
{
  UINT8                             RegRead;
  
  /* RegRead = MmioRead8 ((UINTN)&TisReg->Access); */
  RegRead = TisReg->Access(0);
  ...
  )
{
  ...
  do {
	...
	*BurstCount = (UINTN)TisReg->BurstCount();
    if (*BurstCount != 0) {
      return EFI_SUCCESS;
    }
    MicroSecondDelay (30);
  ...
  )
{
 ...
 /* MmioWrite8((UINTN)&TisReg->Status, TIS_PC_STS_READY);*/
  TisReg->Status(TIS_PC_STS_READY);
  ...
  )
{
  ...
  TisReg->Access(TIS_PC_ACC_RQUUSE);
  //
